...
def _get_pt2ms_mappings(m, prefix=""):
...
def _convert_state_dict(m, state_dict_pt, prefix=""):
    ...
    pt2ms_mappings = _get_pt2ms_mappings(m, prefix)
    state_dict_ms = {}
    while state_dict_pt:
...
class MSPreTrainedModel(nn.Cell, ModuleUtilsMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    def _load_pretrained_model(
        ...
        loaded_keys = [
            _get_pt2ms_mappings(model, (prefix + ".") if prefix in k else "").get(k, (k, None))[0] for k in loaded_keys
        ]
