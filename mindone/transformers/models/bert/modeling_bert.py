    ...
    def construct(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            if self.position_embedding_type == "relative_key":
                relative_position_scores = ops.matmul(query_layer.unsqueeze(3), positional_embedding.permute(0, 2, 1)).squeeze(3) # "bhld,lrd->bhlr"
                attention_scores = attention_scores + relative_position_scores
            elif self.position_embedding_type == "relative_key_query":
                relative_position_scores_query = ops.matmul(query_layer.unsqueeze(3), positional_embedding.permute(0, 2, 1)).squeeze(3) # "bhld,lrd->bhlr"
                relative_position_scores_key = (key_layer[:, :, None, :, :] * positional_embedding[None, None, :, :, :]).sum(-1)  # "bhrd,lrd->bhlr"
                attention_scores = attention_scores + relative_position_scores_query + relative_position_scores_key

        attention_scores = attention_scores / ops.sqrt(ms.tensor(self.attention_head_size, dtype=attention_scores.dtype))
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, attention_mask=None, **model_kwargs):
        ...
        dummy_token = ops.full(
            (effective_batch_size, 1), self.pad_token_id, dtype=ms.int64
        )
