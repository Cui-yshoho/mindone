...
class SanaModulatedNorm(nn.Cell):
    ...
    def construct(
        self, hidden_states: ms.Tensor, temb: ms.Tensor, scale_shift_table: ms.Tensor
    ) -> ms.Tensor:
...
class SanaVideoTransformerBlock(nn.Cell):
    r"""
    ...
    def __init__(
        ...
        self.scale_shift_table = ms.Parameter(mint.randn(6, dim) / dim**0.5)
...
class SanaVideoTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, AttentionMixin):
    ...
    def __init__(
        ...
        self.proj_out = mint.nn.Linear(inner_dim, math.prod(patch_size) * out_channels)
