...
class StableAudioGaussianFourierProjection(nn.Cell):
    ...
    def __init__(
        ...
        super().__init__()
        self.weight = ms.Parameter(ops.randn(embedding_size) * scale, requires_grad=False)
        self.log = log
        ...
        if set_W_to_weight:
            # to delete later
            del self.weight
            self.W = ms.Parameter(ops.randn(embedding_size) * scale, requires_grad=False)
            self.weight = self.W
            del self.W
...
class StableAudioDiTBlock(nn.Cell):
    ...
    def __init__(
        ...
        # 1. Self-Attn
        self.norm1 = LayerNorm(dim, elementwise_affine=True, eps=norm_eps)
        ...
        # 2. Cross-Attn
        self.norm2 = LayerNorm(dim, norm_eps, True)
        ...
        # 3. Feed-forward
        self.norm3 = LayerNorm(dim, norm_eps, True)
        self.ff = FeedForward(
            dim,
            dropout=dropout,
    ...
    def construct(
        ...
        rotary_embedding: Optional[ms.Tensor] = None,
    ) -> ms.Tensor:
...
class StableAudioDiTModel(ModelMixin, ConfigMixin):
    ...
    def construct(
        ...
        if attention_mask is not None:
            prepend_mask = ops.ones((hidden_states.shape[0], 1), dtype=ms.bool_)
