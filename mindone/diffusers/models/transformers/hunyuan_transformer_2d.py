...
class FP32LayerNorm(LayerNorm):
    def construct(self, inputs: ms.Tensor) -> ms.Tensor:
        ...
        return self.layer_norm(inputs.float(), self.weight.float(), self.bias.float()).to(origin_dtype)
...
class HunyuanDiT2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.text_embedding_padding = ms.Parameter(
            ops.randn((text_len + text_len_t5, cross_attention_dim), dtype=ms.float32), name="text_embedding_padding"
        )
