...
class BriaFiboAttention(ms.nn.Cell, AttentionModuleMixin):
...
class BriaFiboEmbedND(ms.nn.Cell):
...
class BriaFiboSingleTransformerBlock(nn.Cell):
...
class BriaFiboTextProjection(nn.Cell):
...
class BriaFiboTransformerBlock(nn.Cell):
    def __init__(
...
class BriaFiboTimesteps(nn.Cell):
    def __init__(
...
class BriaFiboTimestepProjEmbeddings(nn.Cell):
    ...
    def construct(self, timestep, dtype):
