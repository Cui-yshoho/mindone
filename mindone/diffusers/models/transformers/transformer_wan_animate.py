...
class FusedLeakyReLU(nn.Cell):
    ...
    def construct(self, x: ms.Tensor, channel_dim: int = 1) -> ms.Tensor:
...
class MotionConv2d(nn.Cell):
    def __init__(
        ...
        self.weight = ms.Parameter(mint.randn(out_channels, in_channels, kernel_size, kernel_size))
        ...
        if bias and not self.use_activation:
            self.bias = ms.Parameter(mint.zeros(out_channels))
        else:
            self.bias = None
...
class MotionLinear(nn.Cell):
    def __init__(
        ...
        self.weight = ms.Parameter(mint.randn(out_dim, in_dim))
        self.scale = 1 / math.sqrt(in_dim)

        # If an activation is present, the bias will be fused to it
        if bias and not self.use_activation:
            self.bias = ms.Parameter(mint.zeros(out_dim))
        else:
            self.bias = None
    ...
    def construct(self, input: ms.Tensor, channel_dim: int = 1) -> ms.Tensor:
...
class MotionEncoderResBlock(nn.Cell):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class WanAnimateMotionEncoder(nn.Cell):
    def __init__(
        self,
        size: int = 512,
        style_dim: int = 512,
        ...
        self.motion_network = nn.CellList(linears)

        self.motion_synthesis_weight = ms.Parameter(mint.randn(out_dim, motion_dim))

    def construct(self, face_image: ms.Tensor, channel_dim: int = 1) -> ms.Tensor:
...
class WanAnimateFaceEncoder(nn.Cell):
    def __init__(
        self,
        in_dim: int,
        out_dim: int,
        ...
        self.norm3 = mint.nn.LayerNorm(hidden_dim, eps, elementwise_affine=False)

        self.out_proj = mint.nn.Linear(hidden_dim, out_dim)

        self.padding_tokens = ms.Parameter(mint.zeros((1, 1, 1, out_dim)))
...
class WanAnimateFaceBlockAttnProcessor:
    ...
    def __call__(
        ...
        query = unflatten(query, 2, (attn.heads, -1))  # [B, S, H * D] --> [B, S, H, D]
        ...
        query = unflatten(query, 1, (T, -1)).flatten(0, 1)  # [B, S, H, D] --> [B * T, S / T, H, D]
        ...
        hidden_states = unflatten(hidden_states, 0, (B, T)).flatten(1, 2)
...
class WanAnimateFaceBlockCrossAttention(nn.Cell, AttentionModuleMixin):
    ...
    def __init__(
        ...
        self.pre_norm_q = mint.nn.LayerNorm(dim, eps, elementwise_affine=False)
        self.pre_norm_kv = mint.nn.LayerNorm(dim, eps, elementwise_affine=False)
        ...
        self.to_out = mint.nn.Linear(self.inner_dim, dim, bias=True)
    ...
    def fuse_projections(self):
        ...
        if self.cross_attention_dim_head is None:
            ...
            with no_init_parameters():
                self.to_qkv = mint.nn.Linear(in_features, out_features, bias=True)
            ...
            out_features, in_features = concatenated_weights.shape
            with no_init_parameters():
                self.to_kv = mint.nn.Linear(in_features, out_features, bias=True)
        ...
        if self.added_kv_proj_dim is not None:
            ...
            out_features, in_features = concatenated_weights.shape
            with no_init_parameters():
                self.to_added_kv = mint.nn.Linear(in_features, out_features, bias=True)
...
class WanAnimateTransformer3DModel(
    ...
    def construct(
        ...
        Args:
            hidden_states (`ms.Tensor` of shape `(B, 2C + 4, T + 1, H, W)`):
            ...
            encoder_hidden_states_image (`ms.Tensor`):
                CLIP visual features of the reference (character) image.
            pose_hidden_states (`ms.Tensor` of shape `(B, C, T, H, W)`):
                Pose video latents. TODO: description
            face_pixel_values (`ms.Tensor` of shape `(B, C', S, H', W')`):
        ...
        motion_vec = mint.cat(motion_vec_batches)
        ...
        pad_face = mint.zeros_like(motion_vec[:, :1])
