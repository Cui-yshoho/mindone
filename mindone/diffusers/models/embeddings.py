...
def get_1d_rotary_pos_embed(
    ...
    if use_real and repeat_interleave_real:
        ...
        freqs_cos = ops.cat([freqs.cos(), freqs.cos()], axis=-1)  # [S, D]
        freqs_sin = ops.cat([freqs.sin(), freqs.sin()], axis=-1)  # [S, D]
        return freqs_cos, freqs_sin
    else:
...
def apply_rotary_emb(
    ...
    if use_real:
        ...
        if use_real_unbind_dim == -1:
            ...
            x_rotated = ops.stack([-x_imag, x_real], axis=-1).flatten(3)
        ...
        x_rotated = ops.view_as_complex(x.float().reshape(*x.shape[:-1], -1, 2))
