...
class Snake1d(nn.Cell):
    ...
    def __init__(self, hidden_dim, logscale=True):
        super().__init__()
        self.alpha = ms.Parameter(ops.zeros((1, hidden_dim, 1)))
        self.beta = ms.Parameter(ops.zeros((1, hidden_dim, 1)))
...
class OobleckResidualUnit(nn.Cell):
    ...
    def __init__(self, dimension: int = 16, dilation: int = 1):
        ...
        self.conv1 = WeightNorm(
            nn.Conv1d(
                dimension, dimension, kernel_size=7, dilation=dilation, padding=pad, pad_mode="pad", has_bias=True
            )
        )
...
class OobleckDiagonalGaussianDistribution(object):
    def __init__(self, parameters: ms.Tensor, deterministic: bool = False):
        ...
        self.mean, self.scale = parameters.chunk(2, axis=1)
...
class OobleckEncoder(nn.Cell):
    ...
    def __init__(self, encoder_hidden_size, audio_channels, downsampling_ratios, channel_multiples):
        ...
        self.conv1 = WeightNorm(
            nn.Conv1d(audio_channels, encoder_hidden_size, kernel_size=7, padding=3, pad_mode="pad", has_bias=True)
        )
        ...
        self.conv2 = WeightNorm(
            nn.Conv1d(d_model, encoder_hidden_size, kernel_size=3, padding=1, pad_mode="pad", has_bias=True)
        )
...
class OobleckDecoder(nn.Cell):
    ...
    def __init__(self, channels, input_channels, audio_channels, upsampling_ratios, channel_multiples):
        ...
        self.conv2 = WeightNorm(
            nn.Conv1d(channels, audio_channels, kernel_size=7, padding=3, has_bias=False, pad_mode="pad")
        )
...
class AutoencoderOobleck(ModelMixin, ConfigMixin):
    ...
    def _decode(self, z: ms.Tensor, return_dict: bool = False) -> Union[OobleckDecoderOutput, ms.Tensor]:
        dec = self.decoder(z)
    ...
    def decode(self, z: ms.Tensor, return_dict: bool = False, generator=None) -> Union[OobleckDecoderOutput, ms.Tensor]:
        """
...
def norm_except_dim(v, pow, dim):
    if dim == -1:
        return ms.numpy.norm(v, pow)
    elif dim == 0:
        output_size = (v.shape[0],) + (1,) * (v.ndim - 1)
        return ms.numpy.norm(v.view((v.shape[0], -1)), pow, 1).view(output_size)
    elif dim == (v.ndim - 1):
        output_size = (1,) * (v.ndim - 1) + (v.shape[v.ndim - 1])
        return ms.numpy.norm(v.view((-1, v.shape[v.ndim - 1])), pow, 0).view(output_size)
    else:
...
class WeightNorm(nn.Cell):
    def __init__(self, module, dim: int = 0):
        ...
        self.param_g = ms.Parameter(ms.Tensor(norm_except_dim(self.module.weight, 2, dim)))
        self.param_v = ms.Parameter(ms.Tensor(self.module.weight.data))
