...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...     "Kwai-Kolors/Kolors-diffusers", variant="fp16", mindspore_dtype=ms.float16
        ... )
        >>> url = (
...
class KolorsImg2ImgPipeline(DiffusionPipeline, StableDiffusionXLLoraLoaderMixin, IPAdapterMixin):
    r"""
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            for tokenizer, text_encoder in zip(tokenizers, text_encoders):
                ...
                attention_mask_all = text_inputs["attention_mask"].all()
                output = text_encoder(
                    input_ids=ms.tensor(text_inputs["input_ids"]),
                    attention_mask=ms.tensor(text_inputs["attention_mask"]),
                    position_ids=ms.tensor(text_inputs["position_ids"]),
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None and zero_out_negative_prompt:
            ...
            for tokenizer, text_encoder in zip(tokenizers, text_encoders):
                ...
                uncond_attention_mask_all = uncond_input["attention_mask"].all()
                output = text_encoder(
                    input_ids=ms.tensor(uncond_input["input_ids"]),
                    attention_mask=ms.tensor(uncond_input["attention_mask"]),
                    position_ids=ms.tensor(uncond_input["position_ids"]),
                    output_hidden_states=True,
                    attention_mask_all=uncond_attention_mask_all,
                )
    ...
    def get_guidance_scale_embedding(
        self, w: ms.Tensor, embedding_dim: int = 512, dtype: ms.dtype = ms.float32
    ) -> ms.Tensor:
        """
