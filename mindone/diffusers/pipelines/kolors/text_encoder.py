...
class CoreAttention(nn.Cell):
    ...
    def construct(self, query_layer, key_layer, value_layer, attention_mask):
        ...
        if attention_mask is not None:
            attention_scores = attention_scores.masked_fill(attention_mask.bool(), dtype_to_min(attention_scores.dtype))
...
def apply_rotary_pos_emb(x: ms.Tensor, rope_cache: ms.Tensor) -> ms.Tensor:
    # x: [sq, b, np, hn]
    sq, _b, np, _hn = x.shape[0], x.shape[1], x.shape[2], x.shape[3]
