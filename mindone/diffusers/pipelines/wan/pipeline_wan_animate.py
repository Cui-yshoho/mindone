...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> import mindspore as ms
        >>> import numpy as np
        >>> from mindone.diffusers import WanAnimatePipeline
        >>> from mindone.diffusers.utils import export_to_video, load_image, load_video
...
class WanAnimatePipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def encode_prompt(
        ...
        Args:
            ...
            num_videos_per_prompt (`int`, *optional*, defaults to 1):
                Number of videos that should be generated per prompt. mindspore device to place the resulting embeddings on
    ...
    def prepare_prev_segment_cond_latents(
        self,
        prev_segment_cond_video: Optional[ms.Tensor] = None,
        background_video: Optional[ms.Tensor] = None,
        mask_video: Optional[ms.Tensor] = None,
        batch_size: int = 1,
        ...
        full_segment_cond_video = mint.cat([prev_segment_cond_video, remaining_segment], dim=2)
        ...
        if isinstance(generator, list):
            ...
            prev_segment_cond_latents = mint.cat(prev_segment_cond_latents)
        else:
            prev_segment_cond_latents = retrieve_latents(
                self.vae, self.vae.encode(full_segment_cond_video)[0], generator, sample_mode
            )
        # Standardize latents in preparation for Wan VAE encode
        latents_mean = (
        ...
        prev_segment_cond_latents = mint.cat([prev_segment_cond_mask, prev_segment_cond_latents], dim=1)
        return prev_segment_cond_latents

    def prepare_pose_latents(
        self,
        pose_video: ms.Tensor,
        batch_size: int = 1,
        sample_mode: int = "argmax",
        generator: Optional[Union[np.random.Generator, List[np.random.Generator]]] = None,
        dtype: Optional[ms.Type] = None,
    ...
    def __call__(
        ...
        for _ in range(num_segments):
            ...
            reference_latents = mint.cat([reference_image_latents, prev_segment_cond_latents], dim=2)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    latent_model_input = mint.cat([latents, reference_latents], dim=1).to(transformer_dtype)
        ...
        if not output_type == "latent":
            video = mint.cat(all_out_frames, dim=2)[:, :, :cond_video_frames]
