...
class MarigoldImageProcessor(ConfigMixin):
    ...
    @staticmethod
    def expand_tensor_or_array(images: Union[ms.Tensor, np.ndarray]) -> Union[ms.Tensor, np.ndarray]:
        """
    ...
    @staticmethod
    def resize_antialias(image: ms.Tensor, size: Tuple[int, int], mode: str, is_aa: Optional[bool] = None) -> ms.Tensor:
        if not ops.is_tensor(image):
    ...
    @staticmethod
    def resize_to_max_edge(image: ms.Tensor, max_edge_sz: int, mode: str) -> ms.Tensor:
        if not ops.is_tensor(image):
            raise ValueError(f"Invalid input type={type(image)}.")
        if not ops.is_floating_point(image):
            raise ValueError(f"Invalid input dtype={image.dtype}.")
    ...
    @staticmethod
    def pad_image(image: ms.Tensor, align: int) -> Tuple[ms.Tensor, Tuple[int, int]]:
        if not ops.is_tensor(image):
            raise ValueError(f"Invalid input type={type(image)}.")
        if not ops.is_floating_point(image):
            raise ValueError(f"Invalid input dtype={image.dtype}.")
    ...
    @staticmethod
    def unpad_image(image: ms.Tensor, padding: Tuple[int, int]) -> ms.Tensor:
        if not ops.is_tensor(image):
            raise ValueError(f"Invalid input type={type(image)}.")
        if not ops.is_floating_point(image):
            raise ValueError(f"Invalid input dtype={image.dtype}.")
    ...
    def load_image_canonical(
        image: Union[ms.Tensor, np.ndarray, Image.Image],
        ...
        if isinstance(image, (np.ndarray, ms.Tensor)):
    ...
    @staticmethod
    def check_image_values_range(image: ms.Tensor) -> None:
        if not ops.is_tensor(image):
            raise ValueError(f"Invalid input type={type(image)}.")
        if not ops.is_floating_point(image):
            raise ValueError(f"Invalid input dtype={image.dtype}.")
    ...
    def preprocess(
        ...
        if isinstance(image, list):
            ...
            for i, img in enumerate(image):
                img = self.load_image_canonical(img, dtype)  # [N,3,H,W]
                if images is None:
                    images = img
                else:
            ...
            image = self.load_image_canonical(image, dtype)  # [N,3,H,W]
    ...
    def colormap(
        ...
        if _force_method is None and ops.is_tensor(image) and cmap == "Spectral":
    ...
    def export_depth_to_16bit_png(
        depth: Union[np.ndarray, ms.Tensor, List[np.ndarray], List[ms.Tensor]],
        val_min: float = 0.0,
        val_max: float = 1.0,
    ) -> Union[PIL.Image.Image, List[PIL.Image.Image]]:
        def export_depth_to_16bit_png_one(img, idx=None):
            prefix = "Depth" + (f"[{idx}]" if idx else "")
            if not isinstance(img, np.ndarray) and not ops.is_tensor(img):
                raise ValueError(f"{prefix}: unexpected type={type(img)}.")
            if img.ndim != 2:
                raise ValueError(f"{prefix}: unexpected shape={img.shape}.")
