...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import FluxKontextInpaintPipeline
        ...
        >>> image = pipe(prompt=prompt, image=source, mask_image=mask, strength=1.0)[0][0]
...
class FluxKontextInpaintPipeline(
    ...
    def _encode_vae_image(self, image: ms.Tensor, generator: np.random.Generator):
        if isinstance(generator, list):
            image_latents = [
                retrieve_latents(
                    self.vae, self.vae.encode(image[i : i + 1])[0], generator=generator[i], sample_mode="argmax"
                )
            ...
            image_latents = retrieve_latents(
                self.vae, self.vae.encode(image)[0], generator=generator, sample_mode="argmax"
            )
    ...
    def enable_vae_slicing(self):
        ...
        """
        depr_message = f"Calling `enable_vae_slicing()` on a `{self.__class__.__name__}` is deprecated and this method will be removed in a future version. Please use `pipe.vae.enable_slicing()`."  # noqa: E501
        deprecate(
            "enable_vae_slicing",
            "0.40.0",
            depr_message,
    ...
    def disable_vae_slicing(self):
        ...
        """
        depr_message = f"Calling `disable_vae_slicing()` on a `{self.__class__.__name__}` is deprecated and this method will be removed in a future version. Please use `pipe.vae.disable_slicing()`."  # noqa: E501
        deprecate(
            "disable_vae_slicing",
            "0.40.0",
            depr_message,
    ...
    def enable_vae_tiling(self):
        ...
        """
        depr_message = f"Calling `enable_vae_tiling()` on a `{self.__class__.__name__}` is deprecated and this method will be removed in a future version. Please use `pipe.vae.enable_tiling()`."  # noqa: E501
        deprecate(
            "enable_vae_tiling",
            "0.40.0",
            depr_message,
    ...
    def disable_vae_tiling(self):
        ...
        computing decoding in one step.
        """
        depr_message = f"Calling `disable_vae_tiling()` on a `{self.__class__.__name__}` is deprecated and this method will be removed in a future version. Please use `pipe.vae.disable_tiling()`."  # noqa: E501
        deprecate(
            "disable_vae_tiling",
            "0.40.0",
            depr_message,
    ...
    def prepare_latents(
        ...
        image_reference: Optional[ms.Tensor] = None,
    ):
        ...
        if image_reference is not None:
            image_reference = image_reference.to(dtype=dtype)
            ...
            if batch_size > image_reference_latents.shape[0] and batch_size % image_reference_latents.shape[0] == 0:
                ...
                image_reference_latents = mint.cat([image_reference_latents] * additional_image_per_prompt, dim=0)
                ...
                image_reference_latents = mint.cat([image_reference_latents], dim=0)

        latent_ids = self._prepare_latent_image_ids(batch_size, height // 2, width // 2, dtype)

        if latents is None:
            noise = randn_tensor(shape, generator=generator, dtype=dtype)
            latents = self.scheduler.scale_noise(image_latents, timestep, noise)
        else:
            noise = latents.to(dtype=dtype)
            latents = noise
        ...
        image_ids = self._prepare_latent_image_ids(batch_size, image_latent_height // 2, image_latent_width // 2, dtype)
        ...
        if image_reference_latents is not None:
            ...
            image_reference_ids = self._prepare_latent_image_ids(
                batch_size, image_reference_latent_height // 2, image_reference_latent_width // 2, dtype
            )
    ...
    def __call__(
        ...
        Args:
            ...
            image_reference (`ms.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[ms.Tensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                `Image`, numpy array or tensor representing an image batch to be used as the starting point for the
                masked area. For both numpy array and mindspore tensor, the expected value range is between `[0, 1]` If
        ...
        init_image = image.to(dtype=ms.float32)

        # 2.1 Preprocess image_reference
        if image_reference is not None and not (
            isinstance(image_reference, ms.Tensor) and image_reference.shape[1] == self.latent_channels
        ):
            if (
                isinstance(image_reference, list)
                and isinstance(image_reference[0], ms.Tensor)
                and image_reference[0].ndim == 4
            ):
                image_reference = mint.cat(image_reference, dim=0)
        ...
        if image_reference_ids is not None:
            latent_ids = mint.cat([latent_ids, image_reference_ids], dim=0)  # dim 0 is sequence dimension
        elif image_ids is not None:
            latent_ids = mint.cat([latent_ids, image_ids], dim=0)  # dim 0 is sequence dimension
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if image_reference_latents is not None:
                    latent_model_input = mint.cat([latents, image_reference_latents], dim=1)
