...
class HunyuanDiTPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if untruncated_ids.shape[-1] >= text_input_ids.shape[-1] and text_input_ids.shape != untruncated_ids.shape:
    ...
    def __call__(
        ...
        add_time_ids = add_time_ids.to(dtype=prompt_embeds.dtype).tile(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                t_expand = ms.Tensor([t] * latent_model_input.shape[0]).to(
