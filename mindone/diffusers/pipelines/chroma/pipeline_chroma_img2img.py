...
import numpy as np
from transformers import CLIPImageProcessor, T5TokenizerFast

import mindspore as ms
from mindspore import mint

from mindone.transformers import CLIPVisionModelWithProjection, T5EncoderModel
...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import ChromaTransformer2DModel, ChromaImg2ImgPipeline
        ...
        >>> image = pipe(prompt, image=init_image, negative_prompt=negative_prompt)[0][0]
...
class ChromaImg2ImgPipeline(
    ...
    def _get_t5_prompt_embeds(
        ...
        mask_indices = mint.arange(tokenizer_mask.shape[1]).unsqueeze(0).expand((batch_size, -1))
        attention_mask = (mask_indices <= seq_lengths.unsqueeze(1)).to(dtype=dtype)
