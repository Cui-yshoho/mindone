...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...     "stabilityai/stable-diffusion-xl-base-1.0", variant="fp16", mindspore_dtype=ms.float16
        ... )
...
class LeditsAttentionStore:
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str, editing_prompts, PnP=False):
        ...
        if attn.shape[1] <= self.max_size:
            ...
            attn = mint.stack(attn.split(self.batch_size)).permute(1, 0, 2, 3)
            source_batch_size = int(attn.shape[1] // bs)
            self.construct(attn[:, skip * source_batch_size :], is_cross, place_in_unet)

    def construct(self, attn, is_cross: bool, place_in_unet: str):
    ...
    def aggregate_attention(
        ...
        out = mint.stack([mint.cat(x, dim=0) for x in out])
        # average over heads
...
class LeditsGaussianSmoothing:
    def __init__(self):
        ...
        kernel = kernel / mint.sum(kernel)

        # Reshape to depthwise convolutional weight
        kernel = kernel.view(1, 1, *kernel.shape)
        kernel = kernel.tile((1, *[1] * (kernel.dim() - 1)))
    ...
    def __call__(self, input):
        ...
        Apply gaussian filter to input.
            input (ms.Tensor): Input to apply gaussian filter on.
        Returns:
            filtered (ms.Tensor): Filtered output.
        """
        return mint.nn.functional.conv2d(input, weight=self.weight.to(input.dtype))
...
def compute_noise_sde_dpm_pp_2nd(scheduler, prev_latents, latents, timestep, noise_pred, eta):
    def first_order_update(model_output, sample):  # timestep, prev_timestep, sample):
        ...
        mu_xt = (sigma_t / sigma_s * mint.exp(-h)) * sample + (alpha_t * (1 - mint.exp(-2.0 * h))) * model_output

        mu_xt = scheduler.dpm_solver_first_order_update(
            model_output=model_output, sample=sample, noise=mint.zeros_like(sample)
        )

        sigma = sigma_t * mint.sqrt(1.0 - mint.exp(-2 * h))
        if sigma > 0.0:
