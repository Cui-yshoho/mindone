...
ImageInput = Union[
    "PIL.Image.Image", np.ndarray, "ms.Tensor", List["PIL.Image.Image"], List[np.ndarray], List["ms.Tensor"]
]  # noqa
...
class BlipImageProcessor(BaseImageProcessor):
        ...
        Args:
            ...
            return_tensors (`str` or `TensorType`, *optional*):
                The type of tensors to return. Can be one of:
                    ...
                    - `'ms'`: Return a batch of type `ms.Tensor`.
                    - `'np'`: Return a batch of type `np.ndarray`.
    ...
    def postprocess(self, sample: ms.Tensor, output_type: str = "pil"):
        if output_type not in ["ms", "np", "pil"]:
            raise ValueError(
                f"output_type={output_type} is not supported. Make sure to choose one of ['ms', 'np', or 'pil']"
            )
