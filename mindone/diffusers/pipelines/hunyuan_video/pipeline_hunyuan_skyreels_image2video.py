...
EXAMPLE_DOC_STRING = """
    Examples:
        ```python
        >>> import mindspore
        >>> from mindone.diffusers import HunyuanSkyreelsImageToVideoPipeline, HunyuanVideoTransformer3DModel
        ...
        ...     transformer_model_id, mindspore_dtype=mindspore.bfloat16
        ... )
        ...
        >>> negative_prompt = "Aerial view, aerial view, overexposed, low quality, deformation, a poor composition, bad hands, bad teeth, bad eyes, bad limbs, distortion"  # noqa: E501
        >>> image = load_image(
...
class HunyuanSkyreelsImageToVideoPipeline(DiffusionPipeline, HunyuanVideoLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        latents_padding = mint.zeros(padding_shape, dtype=dtype)
        image_latents = mint.cat([image_latents, latents_padding], dim=2)
    ...
    def __call__(
        ...
        image = self.video_processor.preprocess(image, height=height, width=width).to(vae_dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = mint.cat([latent_model_input, latent_image_input], dim=1)
