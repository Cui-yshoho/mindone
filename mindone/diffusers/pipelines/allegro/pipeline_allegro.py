...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import mindspore as ms
        >>> from mindone.diffusers import AutoencoderKLAllegro, AllegroPipeline
        >>> from mindone.diffusers.utils import export_to_video

        >>> vae = AutoencoderKLAllegro.from_pretrained("rhymes-ai/Allegro", subfolder="vae", mindspore_dtype=ms.float32)
        >>> pipe = AllegroPipeline.from_pretrained("rhymes-ai/Allegro", vae=vae, mindspore_dtype=ms.bfloat16)
        ...
        ... )
        >>> video = pipe(prompt, guidance_scale=7.5, max_sequence_length=512)[0][0]
...
class AllegroPipeline(DiffusionPipeline):
    ...
    # Copied from diffusers.pipelines.pixart_alpha.pipeline_pixart_alpha.PixArtAlphaPipeline.encode_prompt with 120->512, num_images_per_prompt->num_videos_per_prompt # noqa: E501
    def encode_prompt(
        self,
        ...
        **kwargs,
        ...
        if "mask_feature" in kwargs:
            deprecation_message = "The use of `mask_feature` is deprecated. It is no longer used in any computation and that doesn't affect the end results. It will be removed in a future version."  # noqa: E501
    ...
    def _prepare_rotary_positional_embeddings(
        ...
        grid_t = grid_t.to(dtype=ms.int64)
