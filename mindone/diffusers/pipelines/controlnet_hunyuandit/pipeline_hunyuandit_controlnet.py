...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        from mindone.diffusers import HunyuanDiT2DControlNetModel, HunyuanDiTControlNetPipeline
        import mindspore as ms

        controlnet = HunyuanDiT2DControlNetModel.from_pretrained(
            "Tencent-Hunyuan/HunyuanDiT-v1.1-ControlNet-Diffusers-Canny", mindspore_dtype=ms.float16
        )

        pipe = HunyuanDiTControlNetPipeline.from_pretrained(
            "Tencent-Hunyuan/HunyuanDiT-v1.1-Diffusers", controlnet=controlnet, mindspore_dtype=ms.float16
        )
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            prompt_embeds = text_encoder(
                ...
                attention_mask=ms.tensor(prompt_attention_mask),
            )
            prompt_embeds = prompt_embeds[0]
            prompt_attention_mask = ms.tensor(prompt_attention_mask).tile((num_images_per_prompt, 1))
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_attention_mask = ms.tensor(negative_prompt_attention_mask).tile((num_images_per_prompt, 1))
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                t_expand = ms.tensor([t.item()] * latent_model_input.shape[0]).to(dtype=latent_model_input.dtype)
