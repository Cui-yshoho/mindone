...
from mindone.transformers.models.smollm3.modeling_smollm3 import SmolLM3ForCausalLM
...
EXAMPLE_DOC_STRING = """
    ...
    ```python
    import mindspore as ms
    from mindone.diffusers import BriaFiboPipeline
    from mindone.diffusers.modular_pipelines import ModularPipeline
...
class BriaFiboPipeline(DiffusionPipeline):
    ...
    def get_prompt_embeds(
        ...
        if all(p == "" for p in prompt):
            input_ids = mint.full((batch_size, 1), bot_token_id, dtype=ms.int64)
            ...
            input_ids = ms.tensor(tokenized.input_ids)
            attention_mask = ms.tensor(tokenized.attention_mask)

            if any(p == "" for p in prompt):
                empty_rows = ms.tensor([p == "" for p in prompt], dtype=ms.bool)
        ...
        prompt_embeds = mint.cat([hidden_states[-1], hidden_states[-2]], dim=-1)
        prompt_embeds = prompt_embeds.to(dtype=dtype)

        prompt_embeds = prompt_embeds.repeat_interleave(num_images_per_prompt, dim=0)
        hidden_states = tuple(layer.repeat_interleave(num_images_per_prompt, dim=0) for layer in hidden_states)
    ...
    def pad_embedding(prompt_embeds, max_tokens, attention_mask=None):
        ...
        if attention_mask is None:
            attention_mask = mint.ones((batch_size, seq_len), dtype=prompt_embeds.dtype)
        else:
            attention_mask = attention_mask.to(dtype=prompt_embeds.dtype)
        ...
        if max_tokens > seq_len:
            ...
            padding = mint.zeros((batch_size, pad_length, dim), dtype=prompt_embeds.dtype)
            prompt_embeds = mint.cat([prompt_embeds, padding], dim=1)

            mask_padding = mint.zeros((batch_size, pad_length), dtype=prompt_embeds.dtype)
            attention_mask = mint.cat([attention_mask, mask_padding], dim=1)
    ...
    def encode_prompt(
        ...
        if prompt_attention_mask is not None:
            prompt_attention_mask = prompt_attention_mask.to(dtype=prompt_embeds.dtype)

        if negative_prompt_embeds is not None:
            if negative_prompt_attention_mask is not None:
                negative_prompt_attention_mask = negative_prompt_attention_mask.to(dtype=negative_prompt_embeds.dtype)
        ...
        text_ids = mint.zeros((prompt_embeds.shape[0], max_tokens, 3)).to(dtype=dtype)
    ...
    def _prepare_attention_mask(attention_mask):
        attention_matrix = mint.einsum("bi,bj->bij", attention_mask, attention_mask)

        # convert to 0 - keep, -inf ignore
        attention_matrix = mint.where(
            attention_matrix == 1, 0.0, -float("inf")
    ...
    def __call__(
        ...
        if guidance_scale > 1:
            ...
            prompt_layers = [
                mint.cat([negative_prompt_layers[i], prompt_layers[i]], dim=0) for i in range(len(prompt_layers))
            ]
        ...
        latent_attention_mask = mint.ones([latents.shape[0], latents.shape[1]], dtype=latents.dtype)
        if guidance_scale > 1:
            latent_attention_mask = latent_attention_mask.repeat(2, 1)

        attention_mask = mint.cat([prompt_attention_mask, latent_attention_mask], dim=1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                timestep = t.expand((latent_model_input.shape[0],)).to(dtype=latent_model_input.dtype)
        ...
        if output_type == "latent":
            ...
            latents_mean = (
                ms.tensor(self.vae.config.latents_mean).view(1, self.vae.config.z_dim, 1, 1, 1).to(latents_dtype)
            )
            ...
            latents_scaled = mint.cat(latents_scaled, dim=0)
            image = []
            for scaled_latent in latents_scaled:
                curr_image = self.vae.decode(scaled_latent.unsqueeze(0), return_dict=False)[0]
                curr_image = self.image_processor.postprocess(curr_image.squeeze(axis=2), output_type=output_type)
