...
class SplitInferenceModule(nn.Cell):
    r"""
    ...
    Example:
        ...
        >>> input_tensor = ops.randn(42, 1000)
    ...
    def construct(self, *args, **kwargs) -> Union[ms.Tensor, Tuple[ms.Tensor]]:
        ...
        for key in list(kwargs.keys()):
            if key not in self.input_kwargs_to_split or not ops.is_tensor(kwargs[key]):
                continue
            split_inputs[key] = ops.split(kwargs[key], self.split_size, self.split_dim)
            kwargs.pop(key)
        ...
        if isinstance(results[0], ms.Tensor):
            return ops.cat(results, axis=self.split_dim)
...
class AnimateDiffFreeNoiseMixin:
            ...
            for i in range(num_transformer_blocks):
                if isinstance(motion_module.transformer_blocks[i], FreeNoiseTransformerBlock):
                    ...
                    ms.load_param_into_net(
                        motion_module.transformer_blocks[i], basic_transfomer_block.parameters_dict(), strict_load=True
                    )
            ...
            for i in range(num_transformer_blocks):
                if isinstance(motion_module.transformer_blocks[i], FreeNoiseTransformerBlock):
                    ...
                    ms.load_param_into_net(
                        ...
                        free_noise_transfomer_block.parameters_dict(),
                        strict_load=True,
                    )
                    motion_module.transformer_blocks[i].set_chunk_feed_forward(
    ...
    def _encode_prompt_free_noise(
        ...
        prompt_interpolation_embeds = prompt_embeds.new_zeros(shape, dtype=prompt_embeds.dtype)
        ...
        if do_classifier_free_guidance:
            ...
            negative_prompt_interpolation_embeds = negative_prompt_embeds.new_zeros(
                shape, dtype=negative_prompt_embeds.dtype
            )
    ...
    def _prepare_latents_free_noise(
        ...
        if self._free_noise_noise_type == "shuffle_context":
            ...
            latents = ops.cat([latents] * num_repeats, axis=2)
    ...
    def _lerp(self, start_index: int, end_index: int, start_tensor: ms.Tensor, end_tensor: ms.Tensor) -> ms.Tensor:
        ...
        interpolated_tensors = ops.cat(interpolated_tensors)
    ...
    def enable_free_noise(
        ...
        prompt_interpolation_callback: Optional[
            Callable[[DiffusionPipeline, int, int, ms.Tensor, ms.Tensor], ms.Tensor]
        ] = None,
    ) -> None:
        r"""
