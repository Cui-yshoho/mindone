...
EXAMPLE_DOC_STRING = """
    Examples:
        ```python
        >>> import mindspore
        >>> from mindone.diffusers import CogVideoXDPMScheduler, CogVideoXVideoToVideoPipeline
        >>> from mindone.diffusers.utils import export_to_video, load_video

        >>> # Models: "THUDM/CogVideoX-2b" or "THUDM/CogVideoX-5b"
        >>> pipe = CogVideoXVideoToVideoPipeline.from_pretrained("THUDM/CogVideoX-5b", mindspore_dtype=mindspore.bfloat16)
...
class CogVideoXVideoToVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        num_frames = len(video) if latents is None else latents.shape[1]
