            ...
            for attn_type in attn_types:
                ...
                if lora_key == "lora_A":
                    ...
                    for proj_key in diff_attn_proj_keys:
                        converted_state_dict[f"{attn_prefix}.{proj_key}.{lora_key}.weight"] = ms.Parameter(
                            mint.cat([fused_qkv_weight]), name=f"{attn_prefix}.{proj_key}.{lora_key}"
                        )
                else:
                    sample_q, sample_k, sample_v = mint.chunk(fused_qkv_weight, 3, dim=0)

                    if attn_type == "img_attn":
                        converted_state_dict[f"{attn_prefix}.to_q.{lora_key}.weight"] = ms.Parameter(
                            mint.cat([sample_q]), name=f"{attn_prefix}.to_q.{lora_key}"
                        )
                        converted_state_dict[f"{attn_prefix}.to_k.{lora_key}.weight"] = ms.Parameter(
                            mint.cat([sample_k]), name=f"{attn_prefix}.to_k.{lora_key}"
                        )
                        converted_state_dict[f"{attn_prefix}.to_v.{lora_key}.weight"] = ms.Parameter(
                            mint.cat([sample_v]), name=f"{attn_prefix}.to_v.{lora_key}"
                        )
                    else:
                        converted_state_dict[f"{attn_prefix}.add_q_proj.{lora_key}.weight"] = ms.Parameter(
                            mint.cat([sample_q]), name=f"{attn_prefix}.add_q_proj.{lora_key}"
                        )
                        converted_state_dict[f"{attn_prefix}.add_k_proj.{lora_key}.weight"] = ms.Parameter(
                            mint.cat([sample_k]), name=f"{attn_prefix}.add_k_proj.{lora_key}"
                        )
                        converted_state_dict[f"{attn_prefix}.add_v_proj.{lora_key}.weight"] = ms.Parameter(
                            mint.cat([sample_v]), name=f"{attn_prefix}.add_v_proj.{lora_key}"
                        )
        ...
        for lora_key in lora_keys:
            for attn_type in attn_types:
                ...
                if lora_key == "lora_A":
                    ...
                    for proj_key in diff_attn_proj_keys:
                        converted_state_dict[f"{attn_prefix}.{proj_key}.{lora_key}.weight"] = Parameter(
                            mint.cat([fused_qkv_weight]), name=f"{attn_prefix}.{proj_key}.{lora_key}.weight"
                        )
                else:
                    sample_q, sample_k, sample_v = mint.chunk(fused_qkv_weight, 3, dim=0)

                    if attn_type == "img_attn":
                        converted_state_dict[f"{attn_prefix}.to_q.{lora_key}.weight"] = Parameter(
                            mint.cat([sample_q]), name=f"{attn_prefix}.to_q.{lora_key}.weight"
                        )
                        converted_state_dict[f"{attn_prefix}.to_k.{lora_key}.weight"] = Parameter(
                            mint.cat([sample_k]), name=f"{attn_prefix}.to_k.{lora_key}.weight"
                        )
                        converted_state_dict[f"{attn_prefix}.to_v.{lora_key}.weight"] = Parameter(
                            mint.cat([sample_v]), name=f"{attn_prefix}.to_v.{lora_key}.weight"
                        )
                    else:
                        converted_state_dict[f"{attn_prefix}.add_q_proj.{lora_key}.weight"] = Parameter(
                            mint.cat([sample_q]), name=f"{attn_prefix}.add_q_proj.{lora_key}.weight"
                        )
                        converted_state_dict[f"{attn_prefix}.add_k_proj.{lora_key}.weight"] = Parameter(
                            mint.cat([sample_k]), name=f"{attn_prefix}.add_k_proj.{lora_key}.weight"
                        )
                        converted_state_dict[f"{attn_prefix}.add_v_proj.{lora_key}.weight"] = Parameter(
                            mint.cat([sample_v]), name=f"{attn_prefix}.add_v_proj.{lora_key}.weight"
                        )
