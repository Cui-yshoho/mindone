...
class WanLoopBeforeDenoiser(ModularPipelineBlocks):
    ...
    @ms._no_grad()
    def __call__(self, components: WanModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
...
class WanImage2VideoLoopBeforeDenoiser(ModularPipelineBlocks):
    ...
    @ms._no_grad()
    def __call__(self, components: WanModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
        block_state.latent_model_input = mint.cat([block_state.latents, block_state.first_frame_latents], dim=1).to(
            block_state.dtype
        )
        return components, block_state
...
class WanFLF2VLoopBeforeDenoiser(ModularPipelineBlocks):
    ...
    @ms._no_grad()
    def __call__(self, components: WanModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
        block_state.latent_model_input = mint.cat(
...
class WanLoopDenoiser(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        ...
        for name in guider_input_names:
            inputs.append(InputParam(name=name, required=True, type_hint=ms.Tensor))
        return inputs
    ...
    def __call__(self, components: WanModularPipeline, block_state: BlockState, i: int, t: ms.Tensor) -> PipelineState:
        ...
        for guider_state_batch in guider_state:
            ...
            cond_kwargs = {
                k: v.to(block_state.dtype) if isinstance(v, ms.Tensor) else v
