...
class WanPrepareLatentsStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[InputParam]:
        return [
            ...
            InputParam(
                ...
                required=True,
                type_hint=int,
                description="Number of prompts, the final batch size of model inputs should be `batch_size * num_videos_per_prompt`. Can be generated in input step.",  # noqa
            ),
    ...
    def check_inputs(components, block_state):
        ...
        if block_state.num_frames is not None and (
            ...
            raise ValueError(
                f"`num_frames` has to be greater than 0, and (num_frames - 1) must be divisible by {components.vae_scale_factor_temporal}, but got {block_state.num_frames}."  # noqa
            )
