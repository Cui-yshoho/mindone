...
def prepare_latents_img2img(
    vae, scheduler, image, timestep, batch_size, num_images_per_prompt, dtype, generator=None, add_noise=True
):
    ...
    if image.shape[1] == 4:
        ...
        if hasattr(vae.config, "latents_mean") and vae.config.latents_mean is not None:
            latents_mean = ms.tensor(vae.config.latents_mean).view(1, 4, 1, 1)
        if hasattr(vae.config, "latents_std") and vae.config.latents_std is not None:
            latents_std = ms.tensor(vae.config.latents_std).view(1, 4, 1, 1)
        ...
        if isinstance(generator, list) and len(generator) != batch_size:
            ...
            if image.shape[0] < batch_size and batch_size % image.shape[0] == 0:
                image = mint.cat([image] * (batch_size // image.shape[0]), dim=0)
            ...
            init_latents = [
                retrieve_latents(vae, vae.encode(image[i : i + 1])[0], generator=generator[i])
                for i in range(batch_size)
            ]
            init_latents = mint.cat(init_latents, dim=0)
        else:
            init_latents = retrieve_latents(vae, vae.encode(image)[0], generator=generator)
...
class StableDiffusionXLInputStep(ModularPipelineBlocks):
    ...
    def check_inputs(self, components, block_state):
        ...
        if block_state.prompt_embeds is not None and block_state.pooled_prompt_embeds is None:
            raise ValueError(
                "If `prompt_embeds` are provided, `pooled_prompt_embeds` also have to be passed. Make sure to generate `pooled_prompt_embeds` from the same text encoder that was used to generate `prompt_embeds`."  # noqa
            )
    ...
    def __call__(self, components: StableDiffusionXLModularPipeline, state: PipelineState) -> PipelineState:
        ...
        if block_state.ip_adapter_embeds is not None:
            for i, ip_adapter_embed in enumerate(block_state.ip_adapter_embeds):
                block_state.ip_adapter_embeds[i] = mint.cat(
        ...
        if block_state.negative_ip_adapter_embeds is not None:
            for i, negative_ip_adapter_embed in enumerate(block_state.negative_ip_adapter_embeds):
                block_state.negative_ip_adapter_embeds[i] = mint.cat(
...
class StableDiffusionXLImg2ImgSetTimestepsStep(ModularPipelineBlocks):
    ...
    @staticmethod
    # Copied from diffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl_img2img.
    # StableDiffusionXLImg2ImgPipeline.get_timesteps with self->components
    def get_timesteps(components, num_inference_steps, strength, denoising_start=None):
...
class StableDiffusionXLInpaintPrepareLatentsStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        return [
            ...
            InputParam(
                "strength",
                default=0.9999,
                description="Conceptually, indicates how much to transform the reference `image` (the masked portion of image for inpainting). "
            ...
            InputParam(
                ...
                required=True,
                type_hint=int,
                description="Number of prompts, the final batch size of model inputs should be batch_size * num_images_per_prompt. Can be generated in input step.",  # noqa
            ),
            InputParam(
                "latent_timestep",
                required=True,
                type_hint=ms.Tensor,
                description="The timestep that represents the initial noise level for image-to-image/inpainting generation. Can be generated in set_timesteps step.",  # noqa
            ),
            InputParam(
                "image_latents",
                required=True,
            ...
            InputParam("dtype", type_hint=ms.Type, description="The dtype of the model inputs"),
        ]
    ...
    def intermediate_outputs(self) -> List[str]:
        return [
            OutputParam(
                "latents", type_hint=ms.Tensor, description="The initial latents to use for the denoising process"
            ),
            OutputParam(
                "noise",
                type_hint=ms.Tensor,
    ...
    # Modified from diffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl_inpaint.StableDiffusionXLInpaintPipeline.
    # _encode_vae_image with self->components
    # YiYi TODO: update the _encode_vae_image so that we can use #Coped from
    @staticmethod
    def _encode_vae_image(components, image: ms.Tensor, generator: np.random.Generator):
        latents_mean = latents_std = None
        if hasattr(components.vae.config, "latents_mean") and components.vae.config.latents_mean is not None:
            latents_mean = ms.tensor(components.vae.config.latents_mean).view(1, 4, 1, 1)
        if hasattr(components.vae.config, "latents_std") and components.vae.config.latents_std is not None:
            latents_std = ms.tensor(components.vae.config.latents_std).view(1, 4, 1, 1)
        ...
        if isinstance(generator, list):
            image_latents = [
                retrieve_latents(components.vae, components.vae.encode(image[i : i + 1])[0], generator=generator[i])
                for i in range(image.shape[0])
            ]
            image_latents = mint.cat(image_latents, dim=0)
        else:
            image_latents = retrieve_latents(components.vae, components.vae.encode(image)[0], generator=generator)
    ...
    # Modified from diffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl_inpaint.
    # StableDiffusionXLInpaintPipeline.prepare_latents adding components as first argument
    ...
    def prepare_mask_latents(self, components, mask, masked_image, batch_size, height, width, dtype, generator):
...
class StableDiffusionXLImg2ImgPrepareLatentsStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        return [
            ...
            InputParam("dtype", required=True, type_hint=ms.Type, description="The dtype of the model inputs"),
        ]
...
class StableDiffusionXLPrepareLatentsStep(ModularPipelineBlocks):
    ...
    def prepare_latents(comp, batch_size, num_channels_latents, height, width, dtype, generator, latents=None):
        shape = (
            batch_size,
            num_channels_latents,
...
class StableDiffusionXLImg2ImgPrepareAdditionalConditioningStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        return [
            ...
            InputParam(
                ...
                required=True,
                type_hint=ms.Tensor,
                description="The pooled prompt embeddings to use for the denoising process (used to determine shapes and dtypes for other additional conditioning inputs). Can be generated in text_encoder step.",  # noqa
            ),
            InputParam(
                "batch_size",
                required=True,
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            ...
            OutputParam("timestep_cond", type_hint=ms.Tensor, description="The timestep cond to use for LCM"),
        ]

    @staticmethod
    # Copied from diffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl_img2img.
    # StableDiffusionXLImg2ImgPipeline._get_add_time_ids with self->components
    def _get_add_time_ids(
        ...
        if (
            ...
            raise ValueError(
                f"Model expects an added time embedding vector of length {expected_add_embed_dim}, but a vector of {passed_add_embed_dim} was created. The model has an incorrect config. Please check `unet.config.time_embedding_type` and `text_encoder_2.config.projection_dim`."  # noqa
            )
    ...
    def __call__(self, components: StableDiffusionXLModularPipeline, state: PipelineState) -> PipelineState:
        ...
        if (
            ...
            block_state.guidance_scale_tensor = ms.tensor(components.guider.guidance_scale - 1).repeat(
                block_state.batch_size * block_state.num_images_per_prompt
            )
            ...
            ).to(dtype=block_state.latents.dtype)
...
class StableDiffusionXLControlNetInputStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        return [
            ...
            InputParam(
                ...
                description="The crop coordinates to use for preprocess/postprocess the image and mask, for inpainting task only. Can be generated in vae_encode step.",  # noqa
            ),
        ]
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            OutputParam("controlnet_cond", type_hint=ms.Tensor, description="The processed control image"),
            OutputParam(
...
class StableDiffusionXLControlNetUnionInputStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[Tuple[str, Any]]:
        return [
            ...
            InputParam(
                ...
                required=True,
                type_hint=ms.Tensor,
                description="The initial latents to use for the denoising process. Used to determine the shape of the control images. Can be generated in prepare_latent step.",  # noqa
            ),
            InputParam(
                "batch_size",
                required=True,
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            OutputParam("controlnet_cond", type_hint=List[ms.Tensor], description="The processed control images"),
            OutputParam(
                "control_type_idx",
                type_hint=List[int],
    ...
    def __call__(self, components: StableDiffusionXLModularPipeline, state: PipelineState) -> PipelineState:
        ...
        block_state.control_type = ms.Tensor(block_state.control_type)

        block_state.control_type = block_state.control_type.reshape(1, -1).to(dtype=block_state.dtype)
