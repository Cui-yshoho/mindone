...
def _get_initial_timesteps_and_optionals(
    ...
    timesteps, num_inference_steps = retrieve_timesteps(scheduler, num_inference_steps, sigmas=sigmas, mu=mu)
...
class FluxSetTimestepsStep(ModularPipelineBlocks):
    ...
    def inputs(self) -> List[InputParam]:
        return [
            ...
            InputParam("latents", type_hint=ms.Tensor),
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            ...
            OutputParam("guidance", type_hint=ms.Tensor, description="Optional guidance to be used."),
        ]
...
class FluxImg2ImgSetTimestepsStep(ModularPipelineBlocks):
    ...
    @staticmethod
    # Copied from diffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3_img2img.
    # StableDiffusion3Img2ImgPipeline.get_timesteps with self.scheduler->scheduler
    ...
    def __call__(self, components: FluxModularPipeline, state: PipelineState) -> PipelineState:
        ...
        timesteps, num_inference_steps = self.get_timesteps(scheduler, num_inference_steps, block_state.strength)
        block_state.timesteps = timesteps
        block_state.num_inference_steps = num_inference_steps
        block_state.sigmas = sigmas
        block_state.guidance = guidance
...
class FluxRoPEInputsStep(ModularPipelineBlocks):
    ...
    def __call__(self, components: FluxModularPipeline, state: PipelineState) -> PipelineState:
        ...
        block_state.img_ids = FluxPipeline._prepare_latent_image_ids(None, height // 2, width // 2, dtype)
    ...
    def __call__(self, components: FluxModularPipeline, state: PipelineState) -> PipelineState:
        ...
        if (
            ...
            img_ids = FluxPipeline._prepare_latent_image_ids(
                None, image_latent_height // 2, image_latent_width // 2, dtype
            )
        ...
        height = 2 * (int(block_state.height) // (components.vae_scale_factor * 2))
        width = 2 * (int(block_state.width) // (components.vae_scale_factor * 2))
        latent_ids = FluxPipeline._prepare_latent_image_ids(None, height // 2, width // 2, dtype)

        if img_ids is not None:
            latent_ids = mint.cat([latent_ids, img_ids], dim=0)
