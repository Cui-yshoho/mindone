...
class FluxKontextLoopDenoiser(ModularPipelineBlocks):
    ...
    def __call__(self, components: FluxModularPipeline, block_state: BlockState, i: int, t: ms.Tensor) -> PipelineState:
        ...
        if image_latents is not None:
            latent_model_input = mint.cat([latent_model_input, image_latents], dim=1)
...
class FluxLoopAfterDenoiser(ModularPipelineBlocks):
    ...
    def intermediate_outputs(self) -> List[OutputParam]:
        return [OutputParam("latents", type_hint=ms.Tensor, description="The denoised latents")]

    @ms._no_grad()
    def __call__(self, components: FluxModularPipeline, block_state: BlockState, i: int, t: ms.Tensor):
