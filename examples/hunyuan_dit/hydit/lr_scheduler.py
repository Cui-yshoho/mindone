...
def get_mindspore_optimizer(optimizer):
...
class LRRangeTest(object):
    ...
    Example:
        >>> optimizer = nn.SGD(params=model.trainable_params(), learning_rate=0.1, momentum=0.9)
        >>> scheduler = LRRangeTest(optimizer)
        >>> data_loader = ms.dataset.GeneratorDataset(...)
    ...
    def __init__(
        ...
        self.optimizer = get_mindspore_optimizer(optimizer)

        if isinstance(lr_range_test_min_lr, list) or isinstance(lr_range_test_min_lr, tuple):
            if hasattr(self.optimizer, "group_params"):
                if len(lr_range_test_min_lr) != len(self.optimizer.group_params):
                    raise ValueError(
                        "expected {} lr_range_test_min_lr, got {}".format(
                            len(self.optimizer.group_params), len(lr_range_test_min_lr)
                        )
                    )
            self.min_lr = list(lr_range_test_min_lr)
        else:
            if hasattr(self.optimizer, "group_params"):
                self.min_lr = [lr_range_test_min_lr] * len(self.optimizer.group_params)
            else:
                raise ValueError(f"{self.optimizer} does not have 'group_params' attribute")
    ...
    def _update_optimizer(self, group_lrs):
        for param_group, lr in zip(self.optimizer.group_params, group_lrs):
            param_group["lr"] = lr
    ...
    def step(self, batch_iteration=None):
        ...
        self._last_lr = [group["lr"] for group in self.optimizer.group_params]
...
class OneCycle(object):
    ...
    def _initialize_lr(self, optimizer, cycle_min_lr, cycle_max_lr, decay_lr_rate, last_batch_iteration):
        self.min_lrs = [cycle_min_lr] * len(optimizer.group_params)
        if last_batch_iteration == -1:
            for lr, group in zip(self.min_lrs, optimizer.group_params):
                group["lr"] = lr

        self.max_lrs = [cycle_max_lr] * len(optimizer.group_params)
    ...
    def _initialize_momentum(self, optimizer, cycle_min_mom, cycle_max_mom, decay_mom_rate, last_batch_iteration):
        if not hasattr(optimizer, "betas"):
            ...
            print(
                f"cycle_momentum is disabled because optimizer {optimizer_name} does not support momentum, no betas attribute"
            )
            self.cycle_momentum = False
            return

        self.decay_mom_rate = decay_mom_rate
        self.min_moms = [(cycle_min_mom, 0.99)] * len(optimizer.group_params)
        self.max_moms = [(cycle_max_mom, 0.99)] * len(optimizer.group_params)

        if last_batch_iteration == -1:
            for momentum, group in zip(self.min_moms, optimizer.group_params):
    ...
    def step(self, batch_iteration=None):
        ...
        self.last_batch_iteration = batch_iteration
        for param_group, lr in zip(self.optimizer.group_params, self.get_lr()):
            param_group["lr"] = lr
        self._last_lr = [group["lr"] for group in self.optimizer.group_params]

        if self.cycle_momentum:
            momentums = self.get_mom()
            for param_group, momentum in zip(self.optimizer.group_params, momentums):
...
class WarmupLR(object):
    ...
    def _format_param(self, optimizer, param_value, param_name):
        if isinstance(param_value, list) or isinstance(param_value, tuple):
            if len(param_value) != len(optimizer.group_params):
                raise ValueError(
                    "expected {} value for {}, got {}".format(
                        len(optimizer.group_params), param_name, FileNotFoundError(param_value)
                    )
                )
            return list(param_value)
        return [param_value] * len(optimizer.group_params)
