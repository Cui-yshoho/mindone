...
class GaussianDiffusion:
    ...
    def training_losses(self, x_start, model_kwargs=None, controlnet=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            terms_loss = out_dict[0]
            if self.loss_type == LossType.RESCALED_KL:
                terms_loss *= self.num_timesteps
            ...
            if self.model_var_type in [
                ...
                B, C = x_t.shape[:2]
                model_output_split, model_var_values = ops.split(model_output, C, axis=1)
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                frozen_out = ops.cat([ops.stop_gradient(model_output_split), model_var_values], axis=1)
                terms_vb = self._vb_terms_bpd(
                    x_start=x_start,
                    x_t=x_t,
                    t=t,
                    clip_denoised=False,
                ...
                if self.loss_type == LossType.RESCALED_MSE:
                    ...
                    terms_vb *= self.num_timesteps / 1000.0
            ...
            if self.model_mean_type == ModelMeanType.VELOCITY:
                ...
                if self.model_mean_type == ModelMeanType.START_X:
                    ...
                    target = noise
            raw_mse = ops.stop_gradient(mean_flat((target - model_output_split) ** 2))
            terms_mse = mse_loss_weight * mean_flat((target - model_output) ** 2)
            if terms_vb:
                terms_loss = terms_mse + terms_vb
                terms_raw_loss = raw_mse + ops.stop_gradient(terms_vb)
            else:
                terms_loss = terms_mse
                terms_raw_loss = raw_mse
        else:
