...
class StableDiffusionControlNetPipeline(
    ...
    def __call__(
        ...
        condition = (
            self.vae.diag_gauss_dist.sample(self.vae.encode(image.float())[0])
            .mul(self.vae.config.scaling_factor)
            .half()
        )
        condition = ops.cat([condition] * 2) if do_classifier_free_guidance else condition
