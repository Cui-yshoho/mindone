...
from .constants import NEGATIVE_PROMPT, SAMPLER_FACTORY, TRT_MAX_BATCH_SIZE, TRT_MAX_HEIGHT, TRT_MAX_WIDTH
...
def get_pipeline(args, vae, text_encoder, tokenizer, model, rank, embedder_t5, infer_mode, controlnet, sampler=None):
    """
...
class End2End(object):
    def __init__(self, args, models_root_path):
        ...
        if self.infer_mode in ["fa", "mindspore"]:
            ...
            self.controlnet = HunYuanControlNet(
                self.args,
                input_size=latent_size,
                **model_config,
                log_fn=logger.info,
            ...
            logger.info(f"Loading mindspore model finished")
    ...
    def load_mindspore_weights(self):
        ...
        if self.args.dit_weight is not None:
            ...
            if os.path.isdir(dit_weight):
                ...
                if str(files[0]).split("/")[-1].startswith("pytorch_model_"):
        ...
        if self.args.controlnet_weight is not None:
            controlnet_weight = self.args.controlnet_weight
            if os.path.isdir(controlnet_weight):
                controlnet_dir = controlnet_weight
                controlnet_path = os.path.join(
                    controlnet_dir, f"pytorch_model_{self.args.control_type}_{load_key}.safetensors"
                )
            elif os.path.isfile(controlnet_weight):
            ...
            controlnet_path = controlnet_dir / f"pytorch_model_{self.args.control_type}_{load_key}.safetensors"
        ...
        if "module" in controlnet_state_dict:
            controlnet_state_dict = controlnet_state_dict["module"]
        controlnet_state_dict = convert_state_dict(self.controlnet, controlnet_state_dict)
        controlnet_local_state = {k: v for k, v in self.controlnet.parameters_and_names()}
        for k, v in controlnet_state_dict.items():
            if k in controlnet_local_state:
                v.set_dtype(controlnet_local_state[k].dtype)
            else:
                pass  # unexpect key keeps origin dtype
        ms.load_param_into_net(self.controlnet, controlnet_state_dict, strict_load=True)
