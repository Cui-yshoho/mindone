...
from llava.utils import disable_mindspore_init
...
def eval_model(
    ...
    if image_file is not None:
        ...
        images_tensor = process_images(images, models["image_processor"], models["model"].config).to(dtype=ms.float16)
    else:
        # fomatted input as training data
        image_sizes = [(1024, 1024)]
        images_tensor = ops.zeros(
            (1, 5, 3, models["image_processor"].crop_size["height"], models["image_processor"].crop_size["width"])
        )
        images_tensor = images_tensor.to(dtype=ms.float16)

    input_ids = ms.tensor(
        tokenizer_image_token(prompt, models["tokenizer"], IMAGE_TOKEN_INDEX, return_tensors="np")
    ).unsqueeze(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="/apdcephfs_data_cq5_1/share_300167803/dengxinchi/project/LLaVA/checkpoints/exps/caption/caption_v3_prolr2-5_lr2-5_v5_checkpoint-50000_merge_lora",
    )
