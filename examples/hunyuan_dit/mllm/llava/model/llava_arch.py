    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            if "unpad" in getattr(config, "mm_patch_merge_type", ""):
                self.image_newline = ms.Parameter(ops.zeros(config.hidden_size, dtype=self.dtype), name="image_newline")
        ...
        if getattr(self, "mm_projector", None) is None:
            ...
            if "unpad" in mm_patch_merge_type:
                ...
                self.image_newline = ms.Parameter(
                    ops.randn(self.config.hidden_size, dtype=self.dtype) * embed_std, name="image_newline"
                )
        else:
...
def unpad_image(tensor, original_size):
    ...
    Args:
    tensor (ms.Tensor): The image tensor, assumed to be in CxHxW format.
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
                ...
                for image_idx, image_feature in enumerate(image_features):
                    if image_feature.shape[0] > 1:
                        ...
                        if "unpad" in mm_patch_merge_type:
                            ...
                            image_feature = image_feature.flatten(start_dim=1, end_dim=2).flatten(
                            ...
                            image_feature = image_feature.permute(0, 2, 1, 3, 4)
                        ...
                        if "unpad" in mm_patch_merge_type:
                            image_feature = ops.cat((image_feature, self.model.image_newline[None]), axis=0)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            image_token_indices = (
                [-1]
                + ops.nonzero(cur_input_ids == IMAGE_TOKEN_INDEX).transpose()[0].tolist()
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    cur_new_labels.append(
                        ops.full((cur_image_features.shape[0],), IGNORE_INDEX, dtype=cur_labels.dtype)
                    )
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer):
        ...
        if model_args.mm_use_im_start_end:
            ...
            if num_new_tokens > 0:
                ...
                output_embeddings = self.get_output_embeddings().weight
