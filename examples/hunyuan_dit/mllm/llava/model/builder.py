    ...
    if "llava" in model_name.lower():
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if model.lm_head.weight.shape[0] != token_num:
                model.lm_head.weight = ms.Parameter(ops.zeros((token_num, tokem_dim), dtype=model.dtype))
                model.model.embed_tokens.weight = ms.Parameter(ops.zeros((token_num, tokem_dim), dtype=model.dtype))
        ...
        #         model = LlavaMptForCausalLM.from_pretrained(model_base, config=cfg_pretrained, **kwargs)
        #     else:
        #         tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
        #         cfg_pretrained = AutoConfig.from_pretrained(model_path)
        #         model = LlavaLlamaForCausalLM.from_pretrained(model_base, config=cfg_pretrained, **kwargs)
        ...
        #         model = LlavaMptForCausalLM.from_pretrained(model_path, **kwargs)
